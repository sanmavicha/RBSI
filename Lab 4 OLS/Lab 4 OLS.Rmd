...
```{r data_wd, include=FALSE}
# First, let's set a working directory
setwd("C:/Users/Mateo Chaparro/Dropbox/Documentos/0_Duke/6_SixthYear/Summer 2023/RBSI/Lab 4 OLS")

#Let's load the data
brazil<- read_stata("BRA_LAPOP.dta")

# datachanges
brazil <- brazil %>% mutate(bf = ifelse(bf==1, "Recipient", "Non-Recipient"),
                            race2 = ifelse(race2==1, "Afro-Brazilian", "Other"),
                            vote1b = ifelse(vote1b==1, "Incumbent", "Opposition"),
                            vote2b = ifelse(vote2b==1, "Incumbent", "Opposition"),
                            pt = ifelse(partyid==2, 1, 0),
                            region = ifelse(region==1, "North", 
                                            ifelse(region==2, "Northeast", 
                                                   ifelse(region==3, "Sountheast", 
                                                          ifelse(region==4, "South", "Midwest")))))
brazil <- brazil %>% rename("turnout1a" = "vote1a", "turnout2a" = "vote2a")
brazil <- select(brazil,-c("age2"))
# Exporting the dataset
write.csv(brazil,"BRA_LAPOP.csv", row.names = FALSE)

#Let's load the data
brazil<- read.csv("BRA_LAPOP.csv")
...
-   **turnout1a**: Did the respondent participate in the first round of presidential elections? 1. Yes 0. No
-   **turnout2a**: Did the respondent participate in the runoff presidential elections? 1. Yes 0. No
-   **vote1b**: Vote-choice in the first round: Incumbent or Opposition
-   **vote2b**: Vote-choice in the runoff: Incumbent or Opposition
-   **pt**: the respondent is member of PT

### Creating and Recoding variables

First let's create a dummy variable for being a recipient of the government program Bolsa Familia

```{r var_change, include=TRUE}
...
sum(brazil$bf1==brazil$bf2)

# Tip: is always better work with numbers than strings

# Now let's create indicator variables for race, vote1b, vote2b, and high school or mor. We can do all of them using the same command
brazil <- brazil %>% mutate(race2_rec = ifelse(race2 == "Afro-Brazilian",1,0),
                            vote1b_rec = ifelse(vote1b=="Incumbent", 1,0),
                            vote2b_rec = ifelse(vote2b=="Incumbent", 1,0),
                            hs = ifelse(educ >= 4, 1, 0))
...
brazil <- brazil %>% mutate(partyid_rec = ifelse(partyid==1, "Non-Partisan", 
                                            ifelse(partyid==2, "PT", 
                                                   ifelse(partyid==3, "PMDB", 
                                                          ifelse(partyid==4, "PSDB", "Other")))))
...
brazil <- brazil %>% mutate(schooled_Afro = ifelse(hs==1 & race2_rec==1, 1, 0))
safro <- sum(brazil$schooled_Afro, na.rm = TRUE)
```
...
plot(x=x, y=y, main = "No Relationship", xlab = "Independent Variable", ylab = "Dependent Variable")
abline(lm(y~x), col="green", lwd =3)

x <- rnorm(1000, 10, 1)
y <- rnorm(1000, 1, 0.3) + 3*x

plot(x=x, y=y, main = "Positive Relationship", xlab = "Independent Variable", ylab = "Dependent Variable")
abline(lm(y~x), col="green", lwd =2)

x <- rnorm(1000, 10, 1)
y <- rnorm(1000, 1, 0.3) - 0.5*x

plot(x=x, y=y, main = "NEgative Relationship", xlab = "Independent Variable", ylab = "Dependent Variable")
abline(lm(y~x), col="green", lwd =3)
```
...
model1 <- lm(formula = income ~ race2_rec, data = brazil)
...
model2 <- lm(turnout1a ~ race2_rec + bf1, data=brazil)
summary(model2)
...
model3 <- lm(turnout1a ~ race2_rec + bf1 + pt + income, data=brazil)
summary(model3)

# What is the coefficient for bf1 in model2
coefficients(model2)[2]

# Let's also calculate a model just with the intercept and call it model5
model4 <- lm(turnout1a ~ 1, data=brazil)
summary(model4)
```
...
```{r lm3, echo=FALSE}
model5 <- lm(turnout2a ~ race2_rec + bf1 + pt + income, data=brazil)
summary(model5)$coefficients[,1:3]
```
...
$$ H_o: \beta_i = \beta_j = \beta_k = ... = 0$$ $$ H_a:\beta_i \neq \beta_j \neq \beta_k = ... \neq 0$$
...
# You can choose which variables to present, add info and export
stargazer(model4, model2, model3, type="text",
          title            = "Results Table 1",
          covariate.labels = c("Afro-Brazilian", "Bolsa Familia Recipient", "Partisan of PT",
                               "Income"),
