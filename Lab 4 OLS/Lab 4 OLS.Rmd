...
setwd("C:/Users/Mateo Chaparro/Dropbox/Documentos/0_Duke/5_FifthYear/Summer 2022/RBSI/Labs/Lab 4 OLS")
...
# Now let's transform the race, vote1a, vote2a, and high school or more dummies. We can do all of them using the same command
brazil <- brazil %>% mutate(race2 = ifelse(race2 == "Afro-Brazilian",1,0),
                            vote1b = ifelse(vote1b=="Incumbent", 1,0),
                            vote2b = ifelse(vote2b=="Incumbent", 1,0),
...
brazil <- brazil %>% mutate(partyid = ifelse(partyid==1, "Non-Partisan", 
                                            ifelse(partyid==2, "PT", 
...
brazil <- brazil %>% mutate(schooled_Afro = ifelse(hs==1 & race2==1, 1, 0))
...
# Now let's transform the race, vote1b, vote2b, and high school dummies. We can do all of them using the same command
brazil <- brazil %>% mutate(race2 = ifelse(race2 == "Afro-Brazilian",1,0),
                            vote1b = ifelse(vote1b=="Incumbent", 1,0),
                            vote2b = ifelse(vote2b == "Incumbent", 1,0),
                            hs = ifelse(educ>=4,1,0))
...
# Harder create a variable that identifies respondents who have a at least a high school degree and are Afro-Brazilian. Call this new variable schooled_afro. How many individuals fit this category?
brazil <- brazil %>% mutate(schooled_Afro = ifelse(race2 == 1 & hs == 1, 1, 0))
sum(brazil$schooled_Afro, na.rm = TRUE) # na.rm = TRUE means we're removing the NAs from the calculation
```
...
model2 <- lm(vote1a ~ race2 + bf1, data=brazil)
summary(model2)
...
model3 <- lm(vote1a ~ race2 + bf1 + pt + income, data=brazil)
summary(model3)
...
model4 <- lm(vote1a ~ 1, data=brazil)
summary(model4)
```

```{r lm2_stude, eval=FALSE}
# Calculate this bivariate relationship and call it model 2
model2 <- lm(formula = vote1a ~ race2 + bf1, data=brazil)
...
```{r lm3, echo=FALSE}
model5 <- lm(vote2b ~ race2 + bf1 + pt + income, data=brazil)
