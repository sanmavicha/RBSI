...
anes <- as.data.frame(_____("lab_Interpretation_Data.____"))

# create subset_anes
vars <- c("___", "____", "educ_attain")
sub_anes <- anes[vars]

# Create the new variable
sub_anes <- _____ %>% mutate(edu_black = _____(hisp==1 _ ______,_,_))

# Calculate mean and median
mean_edu_hisp <- _____(____$edu_black)

med_edu_hisp <- _______(___________)
...
For this part of the lab, we will run a **linear probability model (LPM)**. Logit and probit models can also be used to estimate dichotomous variables. However, for ease of interpretation, we will be going with an LPM. If you want to know more about logit and probit models send Gloria or me an email/slack message. 
...
```{r models_st, warning=FALSE, echo=TRUE, message=FALSE, eval=FALSE}
...
# Table
stargazer(_____, ______, type="text",
          title            = "_________",                        # Give a title to the table 
          covariate.labels = c("Black", "Hispanic"),             # Choose the labels for the variables you want to show  
          dep.var.caption  = "OLS Models",                       # Name your columns
          dep.var.labels   = "Voted",                            # Name your dependent variable
          add.lines = list(c("________", "No", "Yes")),          # add lines of info
          keep = c("_____","____"),                              # keep certain variables  
...
Let's run and interpret another interaction model
```{r inter_st, eval=FALSE}
m5 <- ___(_____)
summary(m5)
```
