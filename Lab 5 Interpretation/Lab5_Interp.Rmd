...
```{r anes_model, include=FALSE}
# Remember to set your working directory!
setwd("C:/Users/Mateo Chaparro/Dropbox/Documentos/0_Duke/5_FifthYear/Summer 2022/RBSI/Labs/Lab 5 Interpretation")

# load your data and make it in a data frame format. Always include the extension at the end of the file. 
anes <- as.data.frame(read.csv("lab_Interpretation_Data.csv"))

# create subset_anes
vars <- c("vote", "hisp", "educ_attain")
sub_anes <- anes[vars]
...
m2 <- lm(vote ~ hisp + black + educ_attain + pid7 +income , data = anes)
# Table
...
```{r models, echo=FALSE}
# Run your first model
m1 <- lm(vote ~ black + hisp, data = anes)

# Run your second model
m2 <- lm(vote ~ black + hisp + pid7 + income + age + educ_attain, data = anes)

# Table
stargazer(m1, m2, type="text",
          title            = "Results Table 1",
          covariate.labels = c("Black", "Hispanic"),
...
```{r interaction, eval=FALSE, include=FALSE}
